...
)
{
    ...
    if( !rew_flag )
    {
                ...
                sid_upd_bad )                                                      /* SID_UPD_BAD              --> start/stay  CNG   */
        {
            ...
            total_brate= 0;   /* this zeroing is needed  for  speech_bad, sid_bad frames */
    ...
    /*  now  bfi, total_brate are set by RX-DTX handler::
         ...
         bfi==0, total_brate==0    cng will continue or start(sid_first, evs_sid_bad)
         bfi=0 , total_brate==1750   cng will continue or start( amrwb_sid_bad , rate set to zero after selectCodec)
