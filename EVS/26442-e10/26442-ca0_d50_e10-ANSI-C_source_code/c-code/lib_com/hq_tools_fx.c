        ...
        FOR( sfm = 0; sfm < num_sfm; sfm++ )
        {
               ...
               && L_sub(prev_normq_fx[sfm],L_shl(L_normq_v[sfm],1)) < 0 && sub(bitalloc_var_fx,4915) <= 0)
            {
                ...
                FOR (i = add(sfm_start[sfm],12); i < sfm_end[sfm]; i++)
                {
                    ...
                    test();
                    test();
                    IF(L_sub(abs(L_coeff_out[i]),L_shl(abs(*p_prev_coeff_out),2)) >0
                       || L_sub(abs(L_coeff_out[i]),L_shr(abs(*p_prev_coeff_out),2)) <0
                       || ((R[sfm] == 0 || *prev_R == 0) && add(R[sfm], *prev_R) != 0))
                    {
                        IF(L_coeff_out[i] >0)
                        {
                            L_coeff_out[i] = L_add(L_shr(abs(L_coeff_out[i]),1),L_shr(abs(*p_prev_coeff_out),1));
                            move32();
                        }
                        ELSE
                        {
